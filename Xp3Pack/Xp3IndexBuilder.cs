...
namespace Arc.Ddsi.Xp3Pack
{
    internal class Xp3IndexBuilder
    {
        ...
        public long _chunkOffset;
        public long _fileChunkOffset;
        ...
        private void BeginChunk(uint type)
        {
            _writer.Write(type);
            _writer.Write(0L);
            if (type == ChunkType.File)
            {
                _fileChunkOffset = _index.Length;
            }
            else
            {
                _chunkOffset = _index.Length;
            }
        }

        private void EndChunk(bool fileChunk = false)
        {
            long startOffset = 0;
            if (fileChunk)
            {
                startOffset = _fileChunkOffset;
            }
            else
            {
                startOffset = _chunkOffset;
            }
